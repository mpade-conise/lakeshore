...
export default function AdminDashboard() {
  ...
  const handlePriceChange = (routeKey, val) => {
    setEditingPrices(prev => ({ ...prev, [routeKey]: parseFloat(val) || 0 }));
  };
  ...
  const handleSavePrices = async () => {
    ...
    try {
      ...
      setStatusMessage({ type: 'success', text: 'Pricing Matrix updated successfully.' });
    } catch (err) {
    ...
    try {
      ...
      ledgerData.forEach(record => {
        ...
        if (status === 'SUCCESSFUL' || status === 'PAID') {
          ...
          if (!record.payout_completed) withdrawableBalance += amount;
      ...
      ledgerData.forEach(record => {
        if (record.departing_center) {
          ...
          if (!routeMap[center]) routeMap[center] = { name: center, totalStudents: 0, revenueGenerated: 0 };
  ...
  const handleAdminWithdrawal = async () => {
    if (financialStats.withdrawableBalance <= 0) return;
    ...
    try {
      ...
      if(unliquidatedIds.length > 0) {
        ...
        setStatusMessage({ type: 'success', text: `Reconciliation pipeline resolved.` });
        fetchAdminData();
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6 font-mono text-zinc-300 relative">
          ...
          <div className="xl:col-span-2 space-y-6">
            <div className="border border-zinc-900 bg-zinc-950/40 p-6 rounded-2xl space-y-4">
              <div>
                ...
                <p className="text-xs text-zinc-500">Compare traveler densities across vectors.</p>
              </div>
      ...
      {/* MANIFEST PRINT LAYOUT ENGINE TARGET CONTAINER */}
      <div id="printable-manifest-area">
        <div style={{ textAlign: 'center', marginBottom: '24px', borderBottom: '2px solid #000', paddingBottom: '12px' }}>
          ...
          <p style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px', marginTop: '4px' }}>Official Passenger Allocation Manifest</p>
          <p style={{ fontSize: '10px', color: '#555' }}>Generated: {new Date().toLocaleString()}</p>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead>
            <tr style={{ borderBottom: '2px solid #000', textAlign: 'left', fontWeight: 'bold' }}>
              ...
              <th style={{ padding: '8px 4px', textAlign: 'center' }}>SEAT</th>
              <th style={{ padding: '8px 4px', textAlign: 'right' }}>FARE</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div style={{ marginTop: '40px', display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
          ...
          <div style={{ textAlign: 'right', marginTop: 'auto' }}>
            <p style={{ borderTop: '1px solid #000', width: '200px', paddingTop: '4px' }}>Authorized Signature</p>
          </div>
        </div>
      </div>
